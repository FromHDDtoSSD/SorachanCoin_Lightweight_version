...
json_spirit::Value CRPCTable::getblocktemplate(const json_spirit::Array &params, bool fHelp) {
    ...
    */
    ...
    if (net_node::vNodes.empty())
        throw bitjson::JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "SORA is not connected!");
    if (block_process::manage::IsInitialBlockDownload())
        throw bitjson::JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "SORA is downloading blocks...");
    ...
    // Save
    mapGbtNewBlock[pblock->hashMerkleRoot] = std::make_pair(pblock, pblock->vtx[0].vin[0].scriptSig);
...
json_spirit::Value CRPCTable::submitblock(const json_spirit::Array &params, bool fHelp) {
    ...
    CDataStream ssBlockHeader(hex::ParseHex(header_hex), SER_NETWORK, version::PROTOCOL_VERSION);
    ...
    CDataStream ssTxs(hex::ParseHex(txs_hex), SER_NETWORK, version::PROTOCOL_VERSION);
    ...
    CBlock block;
    block.nVersion = blockheader.nVersion;
    block.hashPrevBlock = blockheader.hashPrevBlock;
    block.hashMerkleRoot = blockheader.hashMerkleRoot;
    block.nTime = blockheader.nTime;
    block.nBits = blockheader.nBits;
    block.nNonce = blockheader.nNonce;
    block.vtx = txsvch.vtx;
    ...
    if(block.vtx[0].vout.size()==0) {
        ...
        block.vtx[0].vout[0].nValue = diff::reward::GetProofOfWorkReward(block.nBits, 0);
        return false;
    }
